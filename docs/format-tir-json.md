...
uses TIR schema version `1.1.0`, canonical collection ordering, compact JSON,
and no trailing newline. Empty optional `description`, `enum`, and `properties`
are omitted; required document and tool collections remain arrays. It does not
produce a companion artifact.
